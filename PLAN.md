...
곡 목록 · 상세 / 하단 고정 플레이어(재생 · 정지 · 이전 · 다음 · 탐색 · 볼륨 · 반복 · 셔플) / 차트(전체 TOP 100, AI 툴별, 장르별) / 로그인 · 회원가입 / 곡 업로드(오디오 · 커버 · 메타데이터 · 프롬프트) / 좋아요 / 내 라이브러리 / 플레이리스트 / 검색(곡명 · 창작자 · 프롬프트 키워드) / 모바일 대응

**장르 값 (확정)** — `케이팝` · `발라드` · `힙합` · `R&B` · `팝(POP)` · `J-POP` · `인디` · `일렉트로닉` · `기타`. 업로드 폼 셀렉트와 차트 장르별 필터 탭에서 이 9종을 사용합니다.
...
`/upload`는 쿼리 파라미터(`?edit=:id`)로 신규 업로드/수정 모드를 겸합니다. 수정 모드 진입 시 기존 메타데이터로 폼이 프리필됩니다. 별도 `/track/:id/edit` 라우트를 만들지 않습니다.
...
**재생 대기열(Queue) 패널 — UI 설계 확정**

대기열 토글 클릭 시 플레이어 위에 오버레이 드롭다운(카드형, 트랙 목록만 나열)을 표시합니다.

- 드래그 재정렬 없음 — 6단계 후보로 이월
- 패널 상태는 라우트와 무관한 UI-only 상태 — `playerStore`가 아닌 로컬 컴포넌트 상태로 관리
- 패널이 열린 상태에서 페이지를 이동해도 패널은 닫힙니다(라우트 변경 감지 시 close)
...
tracks (
  ...
  genre         text,                      -- '케이팝'|'발라드'|'힙합'|'R&B'|'팝(POP)'|'J-POP'|'인디'|'일렉트로닉'|'기타'
...
- **`ai_tool = 'other'` 집계** — `종합 TOP 100`에만 집계됩니다. `Suno TOP 50` / `Udio TOP 50` 탭에는 노출되지 않습니다.
...
- **토스트 사용 범위** — 토스트는 **실패·에러류에만** 사용합니다(업로드 실패, 로그인 실패, 네트워크 오류, 프롬프트 복사 실패 등). 좋아요 토글처럼 UI가 즉시 상태를 반영하는 성공 액션에는 토스트를 띄우지 않습니다. 예외: 업로드 완료 · 프롬프트 복사 성공은 이동/클립보드 동작의 확인 피드백이므로 success 토스트 유지.
...
| 항목 | 언제까지 |
|---|---|
| **AI 툴 뱃지 컬러** — Suno/Udio 실제 브랜드 컬러를 쓰고 있음. 상표 이슈 소지가 있어 자체 팔레트로 교체할지 — **현행 유지 확정. 5단계 배포 전 재검토 예정으로 이월** | 5단계 |
